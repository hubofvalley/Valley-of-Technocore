...
| CI-recorded | Inspect the public [Test workflow runs](https://github.com/hubofvalley/Valley-of-Technocore/actions/workflows/test.yml), open a run, and match its head SHA to the commit under review. The workflow checks out that SHA, installs Node.js 20, and runs `npm test`. | A local run, untested environments, or any claim outside the test suite. A green badge is not a source-authenticity check. |
