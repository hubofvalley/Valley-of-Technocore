...
import { runUniversalVerify } from '../src/verify-cli.js';
...
} else if (command === 'verify' && args.length === 1 && ['--help', '-h'].includes(args[0])) {
  result = await runUniversalVerify(args, process.stdin, process.stdout, process.stderr);
} else if (command === 'verify') result = await runUniversalVerify(args, process.stdin, process.stdout, process.stderr);
