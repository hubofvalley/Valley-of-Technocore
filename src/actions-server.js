...
<div class="grid"><section><div class="panel"><h2>Create action</h2><form id="create"><label>Name</label><input name="name" maxlength="80" required><label>Operation</label><select name="operation" id="operation"></select><p id="description" class="muted"></p><button>Create</button></form></div>
...
async function boot(){catalogue=(await api('/api/operations')).operations;q('#operation').innerHTML=catalogue.map(o=>'<option value="'+esc(o.id)+'">'+esc(o.label)+'</option>').join('');showDescription();await refreshActions()}
