...
test('every runtime module reachable from either entrypoint stays capability-bounded', () => {
  ...
  assert.deepEqual(runtimeFiles.map((file) => file.replace(`${ROOT_DIR}/`, '')).sort(), [
    ...
    'src/format.js', 'src/provenance.js', 'src/receipt-cli.js', 'src/receipt.js', 'src/technocore-message.js'
  ]);
...
test('package has zero runtime dependencies and only declared runtime files', () => {
  ...
  assert.deepEqual(readdirSync(new URL('../src', import.meta.url)).sort(), ['attestation.js', 'batch-cli.js', 'cli.js', 'format.js', 'provenance.js', 'receipt-cli.js', 'receipt.js', 'technocore-message.js']);
