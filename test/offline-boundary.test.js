...
function runInEmptyDirectory(env) {
  const cwd = mkdtempSync(join(tmpdir(), 'valley-technocore-boundary-'));
  try {
    const before = readdirSync(cwd);
    const result = spawnSync(process.execPath, [CLI.pathname, 'create-evidence'], {
      cwd, input: INPUT, encoding: 'utf8', env
    });
...
test('package has zero runtime dependencies and only declared runtime files', () => {
  ...
  assert.deepEqual(readdirSync(new URL('../src', import.meta.url)), ['cli.js']);
  assert.deepEqual(readdirSync(new URL('../bin', import.meta.url)), ['valley-technocore.js']);
});
