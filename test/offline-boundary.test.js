...
const ROOM_EXPORT_CLI = new URL('../bin/valley-technocore-room-export.js', import.meta.url);
...
test('every runtime module reachable from every entrypoint stays capability-bounded', () => {
  const runtimeFiles = reachableRuntimeFiles([CLI, RECEIPT_INTAKE_CLI, ROOM_EXPORT_CLI, ATTESTATION_CLI]);
  assert.deepEqual(runtimeFiles.map((file) => file.replace(`${ROOT_DIR}/`, '')).sort(), [
    'bin/valley-attestation.js', 'bin/valley-technocore-receipt-intake.js', 'bin/valley-technocore-room-export.js', 'bin/valley-technocore.js', 'src/attestation.js', 'src/batch-cli.js', 'src/cli.js',
    'src/format.js', 'src/provenance.js', 'src/receipt-cli.js', 'src/receipt-intake.js', 'src/receipt.js', 'src/room-export.js', 'src/technocore-message.js', 'src/verify-cli.js'
  ]);
...
test('package has zero runtime dependencies and only declared runtime files', () => {
  ...
  assert.deepEqual(readdirSync(new URL('../src', import.meta.url)).sort(), ['attestation.js', 'batch-cli.js', 'cli.js', 'format.js', 'provenance.js', 'receipt-cli.js', 'receipt-intake.js', 'receipt.js', 'room-export.js', 'technocore-message.js', 'verify-cli.js']);
  assert.deepEqual(readdirSync(new URL('../bin', import.meta.url)).sort(), ['valley-attestation.js', 'valley-technocore-receipt-intake.js', 'valley-technocore-room-export.js', 'valley-technocore.js']);
});
...
test('room-export inspection is environment-invariant and writes no files', () => {
  const message = JSON.parse(TECHNOCORE_MESSAGE);
  const input = `${JSON.stringify({
    seq: 1, ts: '2026-09-09T00:00:00.000000Z', from: message.did, text: message.text,
    nonce: Number(message.nonce), sig: message.signature_b64u
  })}\n`;
  const run = (env) => {
    const cwd = mkdtempSync(join(tmpdir(), 'valley-technocore-export-boundary-'));
    try {
      const before = readdirSync(cwd);
      const result = spawnSync(process.execPath, [ROOM_EXPORT_CLI.pathname, 'inspect', '--room', message.room, '--generation', '1'], {
        cwd, input, encoding: 'utf8', env
      });
      return { result, before, after: readdirSync(cwd) };
    } finally { rmSync(cwd, { recursive: true, force: true }); }
